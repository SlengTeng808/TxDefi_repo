...
class TxDefiToolKit(threading.Thread):
    ...
    def __init__(self, disable_social_media = False):
        ...
        read_payer_hash = False

        while not read_payer_hash:
            load_dotenv(wdir + "/.env", override=True)
            #Default Keys      
            #FYI you can also set custom payers in custom strategies
            wallet_encryption = os.getenv('WALLET_ENCRYPTION', "NONE")

            wallet_encryption = SupportEncryption.to_enum(wallet_encryption)
            payer_keys_hash = os.getenv('PAYER_HASH') #TODO Add encryption loading

            if payer_keys_hash and wallet_encryption != SupportEncryption.NONE:
                pwd = Encryption.get_encryption_password(self.APP_NAME, self.PROFILE_NAME) #FYI could prompt for password instead

                if pwd:
                    key_bytes = Encryption.decrypt_wallet_key(payer_keys_hash, pwd, wallet_encryption)

                    if key_bytes:
                        payer_keys_hash = key_bytes.decode('utf-8')        
                else:
                    print(f"No Password found for {self.APP_NAME}:{self.PROFILE_NAME}. Set WALLET_ENCRYPTION to NONE or set a password.")        

            try:
                if payer_keys_hash:
                    default_signer_keypair = Keypair.from_base58_string(payer_keys_hash)
                    read_payer_hash = True
            except:
                print("Invalid PAYER_HASH.")

            if not read_payer_hash:
                TxDefiToolKit.show_env_ui()   
        ...
        if not disable_social_media:
            ...
            if discord_channels_subs and len(discord_channels_subs) > 0:
                discord_channels_subs = [item.strip() for item in discord_channels_subs.split(',')]
        ...
        if use_geyser and rpc_geyser_uri:
            ...
            if rpc_geyser_uri.startswith("ws"):
                ...
                amm_transaction_socket = SubscribeSocket(rpc_geyser_uri, transactions_decoder, globals.topic_incoming_transactions, [transaction_request], False) #atlas doesn't accept pings
            else:
            ...
            amm_logs_socket = SubscribeSocket(rpc_wss_uri, raydium_logs_decoder, globals.topic_amm_program_event, [ray_program_sub_request], False)
            ...
            pump_amm_logs_socket = SubscribeSocket(rpc_wss_uri, pump_amm_logs_decoder, globals.topic_amm_program_event, [pump_amm_program_sub_request], False)
